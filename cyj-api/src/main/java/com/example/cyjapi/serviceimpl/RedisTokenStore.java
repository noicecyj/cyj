...
public class RedisTokenStore implements TokenStore {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        try {
            ...
            if (refreshToken != null && refreshToken.getValue() != null) {
                ...
                redisMethod(conn, refreshToken, refreshToAccessKey, accessToRefreshKey);
            }
            conn.closePipeline();
        } finally {
            conn.close();
    ...
    private void redisMethod(RedisConnection conn, OAuth2RefreshToken refreshToken, byte[] refreshToAccessKey, byte[] accessToRefreshKey) {
    ...
    public void storeRefreshToken(OAuth2RefreshToken refreshToken, OAuth2Authentication authentication) {
        ...
        try {
            ...
            redisMethod(conn, refreshToken, refreshKey, refreshAuthKey);
            conn.closePipeline();
        } finally {
            conn.close();
        }
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
        ...
        return getAuth2AccessTokens(approvalKey);
    }

    @Override
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        byte[] key = serializeKey(CLIENT_ID_TO_ACCESS + clientId);
        return getAuth2AccessTokens(key);
    }

    private Collection<OAuth2AccessToken> getAuth2AccessTokens(byte[] key) {
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try {
